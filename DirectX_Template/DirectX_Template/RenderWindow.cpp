...
bool RenderWindow::Initialize(WindowContainer* pWindowContainer, HINSTANCE hInstance, std::string window_title, std::string window_class, int width, int height)
{
	...
	this->handle = CreateWindowEx(0, //Extended Windows style
		...
		NULL, //Handle to parent of this window. Since this is the first window, it has no parent.
		NULL, //Handle to menu or child window identifier. Can be set to NULL and use menu in...
		this->hInstance, //Handle to the instance of module to be used with this window.
		pWindowContainer); //Param to create window

	if (this->handle == NULL)
	{
		ErrorLogger::Log(GetLastError(), "CreateWindowEX failed for " + this->window_title);
		return false;
	}
...
LRESULT CALLBACK HandleMessageSetup(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NCCREATE:
	{
		...
		//WindowContainter* const pWindow = reinterpret_cast<WindowContainer*>
